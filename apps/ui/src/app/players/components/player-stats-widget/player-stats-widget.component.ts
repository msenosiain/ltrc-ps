...
export class PlayerStatsWidgetComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.filterContextService.filterContext$
      ...
      .subscribe((ctx) => {
        ...
        if (ctx.forcedSport)
          this.selected = { ...this.selected, sport: ctx.forcedSport };
        if (ctx.forcedCategory)
          this.selected = { ...this.selected, category: ctx.forcedCategory };
        this.loadStats();
      });
  }
  ...
  openFilters(): void {
    ...
    this.dialog
      .open(ScopeFilterDialogComponent, {
        width: '320px',
        data: {
          filterContext: this.filterContext,
          selected: { ...this.selected },
        } satisfies ScopeFilterDialogData,
      })
      .afterClosed()
      .subscribe((result: ScopeFilterSelection | undefined) => {
      ...
      .subscribe({
        next: (stats) => {
          ...
          const effectiveCategories = this.selected.category
            ...
            ? (allowedCategories as CategoryEnum[])
            : undefined;
          const effectiveSport =
            this.selected.sport ?? this.filterContext?.forcedSport;
  ...
  private buildBlocks(
    ...
    if (
      !allowedCategories?.length ||
      allowedCategories.includes(CategoryEnum.PLANTEL_SUPERIOR)
    ) {
      const sportGroups: SportGroup[] = [];
      if (rugbyPs && (!sport || sport === SportEnum.RUGBY))
        sportGroups.push({
      ...
      if (hockeyPs && (!sport || sport === SportEnum.HOCKEY))
        sportGroups.push({
    ...
    for (const block of [
      ...
      if (rugbyCats.length)
        sportGroups.push({
          sport: SportEnum.RUGBY,
          label: 'Rugby',
          categories: rugbyCats,
          subtotal: rugbyCats.reduce((s, c) => s + c.count, 0),
        });
      if (hockeyCats.length)
        sportGroups.push({
          sport: SportEnum.HOCKEY,
          label: 'Hockey',
          categories: hockeyCats,
          subtotal: hockeyCats.reduce((s, c) => s + c.count, 0),
        });
